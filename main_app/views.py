...
def report(request):
    return render(request, 'report.html')  # Renders the report.html template

def admin_user_approval(request):
    return render(request, 'admin_user_approval.html')  # Renders admin_user_approval.html template
...
def login_view(request):
    if request.method == 'POST':
        ...
        # Here you can add your authentication logic or save the data
        return HttpResponse(f"Username: {username}, Email: {email}")
    return render(request, 'home.html')  # Render the form for GET requests
